...
// xchacha20
static inline int get_encryption_key(const char* key_file_path, 
...
static inline int call_file_crypto(const char* target_file, const char* source_file,
                                const char* key_file_path, 
                                int (*operation)(const char* target_file, const char* source_file, 
                                    const unsigned char encryption_key[crypto_secretstream_xchacha20poly1305_KEYBYTES]))
{
    ...
    sodium_mlock(encryption_key, crypto_secretstream_xchacha20poly1305_KEYBYTES);
    ...
    sodium_memzero(encryption_key, crypto_secretstream_xchacha20poly1305_KEYBYTES);
    return success;
}
...
int file_sym_enc_encrypt_key_file(const char* target_file, const char* source_file,
                                  const char* key_file_path)
{
    return call_file_crypto(target_file, source_file,
                            key_file_path, file_sym_enc_encrypt_key);
}
...
{
    return call_file_crypto(target_file, source_file,
                            key_file_path, file_sym_enc_decrypt_key);
}

static inline int decrypt_return_status_cleanup(FILE* file_in, FILE* file_out, int ret_value)
{
    fclose(file_in);
    if (file_out != NULL) {
        fclose(file_out);
                             ...
                             const unsigned char encryption_key[crypto_secretstream_xchacha20poly1305_KEYBYTES])
{
    ...
    do {
        ...
        size_t read_length = fread(in_buffer, 1,
        ...
        if (crypto_secretstream_xchacha20poly1305_pull(&state, out_buffer,
            ...
            return decrypt_return_status_cleanup(file_in, file_out, -1);
        }
